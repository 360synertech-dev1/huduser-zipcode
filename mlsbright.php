...
if (filter_var($rawUrl, FILTER_VALIDATE_URL)) {
    ...
    // Try executing the bot up to 2 times
    $attempts = 10;
    $raw_data = '';
    while ($attempts > 0) {
        $raw_data = executeBot($url);
        if ($raw_data !== "No matching elements found.") {
            // Break the loop if matching elements are found
            break;
        }
    ...
    // If both attempts failed, return "No matching elements found"
    if ($attempts === 0 && $raw_data === "No matching elements found.") {
        echo json_encode(['error' => 'No matching elements found.']);
    } else {
